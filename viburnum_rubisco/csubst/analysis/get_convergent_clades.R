...
ancestral_aas <- readr::read_csv(here::here("analyze", "aa_at_sites_ancestral.csv")) %>%
  select(Site, AA_Ancestral) %>%
  rename(codon_site_alignment = Site)

sites <- readr::read_csv(here::here("csubst", "analysis", "csubst_site_merged.csv")) %>%
  filter(OCNany2spe > 0.95) %>%
  mutate(codon_site_alignment = as.numeric(codon_site_alignment) + 31) %>% # all sequences are missing the first 31 codons
  full_join(ancestral_aas) %>%
