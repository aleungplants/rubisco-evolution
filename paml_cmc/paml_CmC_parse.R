...
mlc_M2a_rel <- readr::read_file(here::here("paml_M2a_rel/mlc"))

lnL_CmC <- mlc_CmC %>% 
  stringr::str_extract("lnL(.*)") %>%
  stringr::str_extract("[:punct:][:digit:]+[:punct:][:digit:]+") %>%
  as.numeric()
lnL_M2a_rel <- mlc_M2a_rel %>% 
  stringr::str_extract("lnL(.*)") %>%
  stringr::str_extract("[:punct:][:digit:]+[:punct:][:digit:]+") %>%
  as.numeric()
...
np_CmC <- mlc_CmC %>% stringr::str_extract("(?<=np:)[:digit:]+") %>%
  as.numeric()
np_M2a_rel <- mlc_M2a_rel %>% stringr::str_extract("(?<=np:)[:digit:]+") %>%
  as.numeric()
...
omegas_CmC_fg3 <- mlc_CmC %>% 
  ...
  stringr::str_squish()
omegas_M2a_rel <- mlc_M2a_rel %>% 
  stringr::str_extract("\nw:(.*)") %>%
  stringr::str_squish()

# Model-null model pairs and their comparisons are manually entered
lrt <- tibble(Model = "M2a_rel",
              np = np_M2a_rel,
              AIC = getAIC(lnL_M2a_rel, np_M2a_rel),
              lnL = lnL_M2a_rel,
              K = kappa_M2a_rel,
              w0 = stringr::word(omegas_M2a_rel, -3),
              w1 = stringr::word(omegas_M2a_rel, -2),
              w2 = stringr::word(omegas_M2a_rel, -1),
              p0 = stringr::word(props_M2a_rel, -3),
              p1 = stringr::word(props_M2a_rel, -2),
              p2 = stringr::word(props_M2a_rel, -1),
              ChiSq = NA,
              df = NA,
              p = NA) %>%
  add_row(Model = "BG: warm temperate",
          ...
          ChiSq = 2*(lnL_CmC-lnL_M2a_rel),
          df = np_CmC-np_M2a_rel,
