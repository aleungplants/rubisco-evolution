...
seq_species <- read.csv(here::here("accessions.csv")) %>% # file with accessions
  dplyr::pull(species) %>%
...
exact_taxon_keys <- raw_taxon_keys %>% 
  filter(stringr::str_detect(verbatim_name, canonicalName)) 
mismatches <- raw_taxon_keys %>% 
  filter(!stringr::str_detect(verbatim_name, canonicalName)) %>% 
  select(scientificName, canonicalName, verbatim_name)

FUZZY_SPECIES_KEEP <- mismatches %>% pull(scientificName) # Manually check names for reasonable matches, e.g., off by one or two letters, all look good
