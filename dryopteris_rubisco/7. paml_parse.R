...
here::i_am("7. paml_parse.R")

# PARSE FOR LRT CALCULATIONS AND PARAMETER REPORTING

mlc <- readr::read_file(here::here("mlc")) %>%
...
readr::write_csv(mlc, here::here("paml_site_mlc.csv"))

# PARSE BEB RESULTS

rst <- readr::read_file(here::here("rst"))
...
readr::write_csv(omega, here::here("paml_site_omega.csv"))

# PARSE ANCESTRAL RECONSTRUCTION

rst <- readr::read_file(here::here("paml_reconstruction/rst"))
...
arc_split <- arc_rows %>%
  tidyr::separate_wider_delim(cols = value,
                              names = c("Site", "ARC"),
                              delim = stringr::regex("(?<=^\\s{0,3}[:digit:]{1,3})\\s+")) %>%
  mutate(Site = as.numeric(Site)) %>%
...
readr::write_csv(arc, here::here("paml_site_arc.csv"))
